...
function wats_admin_insert_ticket_custom_field($idvalue)
{
	...
	else if ($idtype != 0 && $idtype != 1 && $idtype != 2 && $idtype != 3 && $idtype != 4 && $idtype != 5)
	{
...
function wats_admin_add_notification_rules_interface($resultsup,$resultadd,$idsup,$idadd,$value,$input)
{
	...
	echo '<input type="submit" class="button-primary" id="'.esc_attr($idsup).'" value="'.__('Remove selected rules','WATS').'" /><div id="'.esc_attr($resultsup).'"></div><br /><br />';
	...
	foreach ($wats_ticket_custom_field_values as $key => $table)
	{
		...
		{
			$code .= '<option value="'.esc_attr($table['meta_key']).'">'.esc_html($table['name']).'</option>';
		}
	}
	if (strlen($code))
	{
	...
	if ($wats_settings['ticket_type_key_enabled'] == 1)
	{
		...
		foreach ($wats_ticket_type as $key => $value)
			echo '<option value="'.esc_attr($key).'">'.esc_html__($value,'WATS').'</option>';
	...
	echo '</table><br />';
	echo '<input type="submit" id="'.esc_attr($idadd).'" value="'.__('Add this rule','WATS').'" class="button-primary" /><div id="'.esc_attr($resultadd).'"></div>';
...
function wats_admin_add_category_interface($resultsup,$resultadd,$idsup,$idadd,$value,$input)
{
	...
	echo '<input type="submit" class="button-primary" id="'.esc_attr($idsup).'" value="'.__('Remove selected categories','WATS').'" /><div id="'.esc_attr($resultsup).'"></div><br /><br />';

	echo '<table class="wats-form-table" cellspacing="1" cellpadding="1">';
	echo '<tr><th><label>'.esc_html(__($value,'WATS')).'</label></th><td>';
	...
	echo '</select></td><td></td></tr>';
	echo '</table><br />';
	echo '<input type="submit" id="'.esc_attr($idadd).'" value="'.__('Add this category','WATS').'" class="button-primary" /><div id="'.esc_attr($resultadd).'"></div>';
...
function wats_admin_add_table_interface($resultsup,$resultadd,$idsup,$idadd,$value,$input)
{
	...
	echo '<input type="submit" class="button-primary" id="'.esc_attr($idsup).'" value="'.__('Remove selected items','WATS').'" /><div id="'.esc_attr($resultsup).'"></div><br /><br />';

	echo '<table class="wats-form-table" cellspacing="1" cellpadding="1">';
	echo '<tr><th><label>'.esc_html(__($value,'WATS')).'</label></th><td><input type="text" name="'.esc_attr($input).'" id="'.esc_attr($input).'" size="30" class="regular-text" /></td><td></td></tr>';
	echo '</table><br />';
	echo '<input type="submit" id="'.esc_attr($idadd).'" value="'.__('Add this entry','WATS').'" class="button-primary" /><div id="'.esc_attr($resultadd).'"></div><br /><br />';
	...
	if (is_array($wats_notification_rules))
	{
		foreach (array_keys($wats_notification_rules) AS $key)
		{
			foreach ($wats_notification_rules[$key] AS $rule => $list)
			{
				...
				echo '<td><input type="checkbox" name="notification_rule_check" id="notification_rule_check" value="'.esc_attr($key).'" /></td>';
				echo '</tr>';
...
function wats_admin_display_options_list($type,$check,$defaultvalue)
{
	...
	if (isset($wats_settings[$type]))
	{
		...
		foreach ($wats_options AS $key => $value)
		{
			...
			echo '<td><input type="checkbox" name="'.esc_attr($check).'" id="'.esc_attr($check).'" value="'.esc_attr($key).'" /></td>';
			echo '</tr>';
    ...
    if ($x == 0)
    {
		if ($type != 'wats_categories' && $type != 'wats_slas')
			...
			$colspan = 3;
        echo '<tr valign="middle"><td colspan="'.esc_attr($colspan).'" style="text-align:center">'.__('No entry','WATS').'</td></tr>';
    }
	echo '</tbody></table><br />';
...
function wats_options_display_frontend_queries_table()
{
	...
	{
		$x = 1;
		$output .= '<tr><td>'.esc_html($key).'</td><td>'.esc_html($table['name']).'</td>';
...
function wats_admin_options_get_custom_field_table_row()
{
	...
	else
	{
		...
		if (isset($table['type']) && $table['type'] == 3)
			$output .= ' selected';
		$output .= '>'.__('Radio button','WATS').'</option><option value="4"';
		if (isset($table['type']) && $table['type'] == 4)
			$output .= ' selected';
		$output .= '>'.__('Checkbox','WATS').'</option><option value="5"';
		if (isset($table['type']) && $table['type'] == 5)
			$output .= ' selected';
		$output .= '>'.__('Integer number','WATS').'</option></select></td>';
...
function wats_options_display_custom_fields_table()
{
	...
	foreach ($wats_ticket_custom_field_values as $key => $table)
	{
		...
		if (!isset($table['type']) || $table['type'] == 0)
			...
			$output .= '<td>'.__('Checkbox','WATS').'</td>';
		else  if ($table['type'] == 5)
			$output .= '<td>'.__('Integer number','WATS').'</td>';
		$output .= '<td>'.$wats_custom_fields_selectors[$table['fsf']].'</td>';
		$output .= '<td>'.$wats_custom_fields_selectors[$table['atef']].'</td>';
		$output .= '<td>'.$wats_custom_fields_selectors[$table['ftdt']].'</td>';
		$output .= '<td>'.$wats_custom_fields_selectors[$table['ftuf']].'</td>';
...
function wats_options_display_custom_fields_selectors_field($table,$id,$selection)
{
	...
	$output = '<select name="'.esc_attr($id).'" id="'.esc_attr($id).'" size="1">';
		...
		if (isset($table['type']) && ($table['type'] == 1 || $table['type'] == 3))
		{
			if (isset($table['values']))
			{
				...
				foreach ($values as $key => $value)
				{
					...
					$output .= ' ></td>';
					$output .= '<td><input type="checkbox" name="customselectorcheck" id="customselectorcheck" value="'.esc_attr($key).'" /></td></tr>';
					$alt = !$alt;
				}
			}
		}
...
function wats_options_manage_ticket_submission_options()
{
	...
	foreach ($userlist AS $userlogin => $username)
	{
		...
		{
			echo '<option value="'.esc_attr($userlogin).'" ';
			if ($userlogin == $wats_settings['wats_guest_user']) echo 'selected';
				echo '>'.esc_html($username).'</option>';
		}
	}
...
function wats_options_manage_ticket_listing_options()
{
	...
	{
        echo '<option value="'.esc_attr($metakey).'" ';
        if ($metakey == $wats_settings['user_selector_order_1']) echo 'selected';
			echo '>'.esc_html($metakey).'</option>';
	}
...
function wats_options_manage_ticket_keys_options()
{
	...
	{
		if (isset($table['type']) && ($table['type'] == 1 || $table['type'] == 3))
			echo '<option value="'.esc_attr($key).'">'.esc_html($table['name']).'</option>';
	}
	echo '</select><br /><br />';
